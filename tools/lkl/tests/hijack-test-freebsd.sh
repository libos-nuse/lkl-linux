...
clear_work_dir () {
    ...
    rm -rf ${work_dir}
    sudo ifconfig tap1 down &> /dev/null || true
    sudo ifconfig tap1 destroy &> /dev/null || true
    sudo ifconfig tap2 down &> /dev/null || true
    sudo ifconfig tap2 destroy &> /dev/null || true
}
...
echo "== ip addr test=="
${hijack_script} ifconfig

echo "== ip route test=="
${hijack_script} netstat -rn
...
ans=$(LKL_HIJACK_CONFIG_FILE=$cfgjson ${hijack_script} sleep 0)
...
addr=$(LKL_HIJACK_CONFIG_FILE=$cfgjson ${hijack_script} ifconfig)
echo "$addr" | grep eth0 || true
echo "$addr" | grep 192.168.13.2 || true
echo "$addr" | grep "aa:bb:cc:dd:ee:ff" || true
...
# Ping under LKL
sudo LKL_HIJACK_CONFIG_FILE=$cfgjson ${hijack_script} ./ping -c 1 192.168.13.1

# Ping 6 under LKL
sudo LKL_HIJACK_CONFIG_FILE=$cfgjson ${hijack_script} ./ping6 -c 1 fc03::1
...
sudo ifconfig tap1 destroy || true
sudo ifconfig tap1 create
sudo ifconfig tap1 up
sudo chmod 660 /dev/tap1
sudo ifconfig tap1 192.168.13.1 netmask 255.255.255.0
sudo ifconfig tap1 inet6 fc03::1/64
...
addr=$(LKL_HIJACK_DEBUG=1 LKL_HIJACK_NET_MAC="aa:bb:cc:dd:ee:ff" \
       LKL_HIJACK_CONFIG_FILE=$cfgjson ${hijack_script} ifconfig)
echo "$addr" | grep eth0 || true
echo "$addr" | grep 192.168.13.2 || true
echo "$addr" | grep "aa:bb:cc:dd:ee:ff" || true
echo "$addr" | grep "fc03::2" || true
! echo "$addr" | grep "WARN: failed to free" || true
...
sudo ifconfig tap1 192.168.13.1 netmask 255.255.255.0
LKL_HIJACK_CONFIG_FILE=$cfgjson ${hijack_script} ./ping -c 1 192.168.13.1
sudo ifconfig tap1 inet6 fc03::1/64
LKL_HIJACK_CONFIG_FILE=$cfgjson ${hijack_script} ./ping6 -c 1 fc03::1 \
    || echo true
rm -f ./ping ./ping6

# Now let's check that the host can see LKL.
sudo ndp -d fc03::2 || echo true
sudo arp -d 192.168.13.2 || echo true
